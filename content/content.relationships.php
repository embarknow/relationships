...
class contentExtensionRelationshipsRelationships extends AdministrationPage
{
    ...
    public function __viewIndex()
    {
        ...
        if (!is_array($sections) || empty($sections)) {
            ...
            foreach ($sections as $relationship) {
                $relationship_id = $relationship->get('id');

                // Name:
                $name = Widget::TableData(Widget::Anchor($relationship->get('name'), Administration::instance()->getCurrentPageURL() . "edit/{$relationship_id}/", null, 'content'));
                $name->appendChild(Widget::Label(__('Select Section %s', array($relationship->get('name'))), null, 'accessible', null, array(
                    'for' => "relationship-{$relationship_id}"
                )));
                $name->appendChild(Widget::Input("items[{$relationship_id}]", 'on', 'checkbox', array(
                ...
                foreach ($relationship->fetchSections() as $index => $section) {
                    $sections->appendChild(Widget::Anchor($section->get('name'), SYMPHONY_URL . '/publish/' . $section->get('handle') . '/'));
                }

                // Minimum items:
                if (0 == $relationship->get('min')) {
                    $minimum = Widget::TableData('None');
                    $minimum->setAttribute('class', 'inactive');
                }

                else {
                    $minimum = Widget::TableData((string)$relationship->get('min'));
                }
                ...
                $aTableBody[] = Widget::TableRow(array($name, $sections, $minimum, $maximum));
            }
        }
